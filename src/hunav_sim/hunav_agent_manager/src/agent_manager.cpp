...
#include "hunav_msgs/msg/agent_goal.hpp"
...
// AMM / mrht simulation -->
void AgentManager::dynamicGoalCallback(const hunav_msgs::msg::AgentGoal::SharedPtr msg)
{
  std::lock_guard<std::mutex> guard(mutex_);
  int id = msg->agent_id;
  if (agents_.find(id) == agents_.end()) {
    RCLCPP_WARN(rclcpp::get_logger("AgentManager"), "Agent id %d not found!", id);
    return;
  }

  agents_[id].sfmAgent.goals.clear();
  sfm::Goal g;
  g.center.setX(msg->goal.position.x);
  g.center.setY(msg->goal.position.y);
  g.radius = msg->radius;
  agents_[id].sfmAgent.goals.push_back(g);
}
// <-- AMM / mrht simulation
